    ...
    pub fn show(
        ...
        let output = scroll.show_rows(ui, row_height, len, |ui, rows| {
            ...
            for hit in rows {
                ...
                let fits = (((rect.right() - text_x) / char_w.max(1.0)).floor().max(0.0) as usize)
                    .min(MAX_ROW_CHARS);
                if let Some(raw) = engine.get_line(line) {
                    let text = clip_chars(&raw, fits);
                    let fg = if level_colors {
                        theme
                    ...
                    let job = row_job(text, query_lower, &font_id, fg);
                    let galley = ui.ctx().fonts_mut(|f| f.layout_job(job));
                    painter.galley(egui::pos2(text_x, text_top), galley, fg);
                }
