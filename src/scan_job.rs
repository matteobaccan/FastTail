...
impl FilterSpec {
    ...
    pub fn matches(&self, line: &str) -> bool {
        !self.excluded(line) && self.included(line)
    }
    ...
    pub fn visible_in_sequence(&self, line: &str, parent_visible: bool) -> (bool, bool) {
        if crate::tail_engine::TailEngine::is_stacktrace_continuation(line) {
            let v = !self.excluded(line) && (self.included(line) || parent_visible);
            (v, parent_visible)
        } else {
...
fn run(
    ...
    let mut eval = |idx: usize, bytes: &[u8], hits: &mut Vec<usize>| -> bool {
        match &spec {
            JobSpec::Index => true,
    ...
    'outer: while pos < range.end_offset {
        ...
        while k + char_bytes <= n {
            ...
            if is_newline {
                ...
                if !eval(line_idx, &line_bytes, &mut hits) {
                    limit_reached = true;
                    break 'outer;
                }
                line_idx += 1;
    ...
    } else if !hits.is_empty() && !send(ScanBatch::Lines(std::mem::take(&mut hits))) {
        return;
    }
    send(ScanBatch::Done {
