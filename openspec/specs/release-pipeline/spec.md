...
The release profile SHALL use thin LTO with one codegen unit only if a documented benchmark of include/exclude filtering and search on a log of at least 1 GB shows the thin build within 5% of the fat build. Otherwise the profile SHALL keep fat LTO. The measurement SHALL be recorded in the change design.
