...
- **Depends on `background-timestamp-scan`.** Opening the strip calls `request_timestamps()`. Without the background scan that call would time a multi-GB file on the interface thread, which is exactly the 0.9.0 known issue; this change is scheduled after 0.9.1 for that reason.
...
- **Search lane from the existing hit list.** When the search lane is on and a query is active, a thin lane above the bars marks, per pixel, the buckets containing hits: each hit's line is looked up in the timestamp cache. The lane is rebuilt only when the hit list changes (keyed by the query, the hit count and the buffer generation, or the search generation counter from `search-results-pane` if that has landed), which costs one lookup per stored hit. Hits are computed over the visible lines, so with a window set they only appear inside it; the tooltip says so.
