...
- [x] 2.1 `AnsiMode` per stream; auto-detection on the first 64 KB at open and on the first 64 KB of each appended chunk until the first SGR hit
- [x] 2.2 `get_line` returns the stripped text in Render/Strip; `get_row` returns the view text with the style runs (Render) or the `␛` substitution for display (Raw)
...
- [x] 2.6 `match_highlight_spans`: ANSI runs after user rules and quick labels, within `MAX_ROW_SPANS`; level fallback only on uncoloured bytes of rows without a whole-row rule match
...
- [x] 3.3 Toolbar selector (combo box) `ANSI: auto → render` offering auto/render/strip/raw and showing the resolved mode; stream bar notice when auto switches to render
