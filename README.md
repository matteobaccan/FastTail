...
| **Time Range Filter** | **`from / to` window, go-to-time, visible span, timeline histogram** | No | Yes | No | `awk` by hand |
...
`📊` next to the fields opens the **timeline histogram** above the rows: bars over the stream's whole time span, each stacked by level — ERROR/FATAL, WARN, INFO, DEBUG/TRACE and lines with no level — in the theme's level colours, with the tallest bar's count as the scale. It counts **every timed line of the stream, whatever the filters**, so it shows what lies outside the current window, which is drawn as a shaded band. **Click** a bar to set the time range to its span, or **drag** across bars for the span between them: the bounds are written into the from/to fields (`to` is the last second of the last bar) and behave exactly like typed ones, combined with the other filters and cleared by `✖`. Hovering a bar shows its span and its counts per level. With `🔍` beside it, a thin lane above the bars marks where the current search hits fall (hits are found among the visible lines, so under a window they only appear inside it). Opening the histogram times the stream like the time range does — in the background above 16 MB, with the bars growing as lines are timed — and a log without usable timestamps gets the same hint as the fields. The histogram is built incrementally in at most 2,048 buckets, one second wide at first and doubling as the span grows (tens of KB per stream), and follows appends, truncation and rewrites. Whether it is shown, and the search lane, are saved in `fasttail.ini` (`timeline_histogram`, `timeline_search_lane`).
